...
| I6′ | **A connector needs room, not just an outline.** Every board's *envelope* — its keepout plus a box per connector for the room the mated cable needs before it can bend — must clear every other envelope, the battery, the SUN and its cradle, the static bay, and the **pitot/static hose corridor**. This is the check print-3 needed and did not have: the MS4525's aft JST landed where the Boost had to go, and both boards individually fitted their footprints perfectly. Three obstacles in that list were absent until they bit — the SUN's shoulder bulkheads were modelled as discs about the bore when `_cradle_plate` actually boxes them over the full height; the static bay was not an obstacle at all, so the Pro Micro sat 3 mm inside its forward wall while the check reported clear; and nothing reserved the ~40 cm³ of bowl the tubing from the barbs to the MS4525 ports consumes. |
| I9 | **No invented holes.** A printed post is legitimate only if it is a measured hole from `BOARD_CALIPERS.md`, a nub or keeper that sheet explicitly calls for, or a tray fixing. v2 generated four corner standoffs per board regardless, which is how the MS4525 got two posts supporting nothing and the Pro Micro got four for holes it does not have. A measured hole whose world Z falls outside the insert-reach band is **demoted to a nub** rather than vetoing the placement — the Boost's upper pair lands above the top flange rail, and a hole you cannot fit an insert into is a nub whether the model says so or not. |
...
| I8 | **Every board must be RESTRAINED** — not merely screwed. The old rule was "at least two holes", which existed to catch a board whose fasteners had been forgotten. With the holes now measured that rule is simply wrong: the MMC5983 has one hole, the BMP581 has two on the same edge, and the Pro Micro has none. What matters is that nothing can rotate or rock — a screwed hole, a support nub, a keeper or a tray all count, and the validator checks the combination, not the count. |
